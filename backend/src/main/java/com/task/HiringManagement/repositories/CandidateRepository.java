...
public interface CandidateRepository extends JpaRepository<Candidate,Long> {
    ...
    @Query(value = "SELECT c.id,c.full_name,c.contact_number,c.birth,c.email FROM candidates c inner join  candidates_skills cs on c.id=cs.candidate_id  where cs.skill_id in (:ids) and LOWER(c.full_name) like CONCAT('%',LOWER(:name) ,'%') group by c.id having count(c.id)=(:id_size) ",nativeQuery = true)
    Page<Candidate> findCandidateComplex(@Param("ids") List<Long> skills,@Param("name") String name,@Param("id_size") Integer idSize, PageRequest pageable);
    @Query(value = "SELECT * FROM candidates c inner join  candidates_skills cs on c.id=cs.candidate_id  where cs.skill_id=:id",nativeQuery = true)
    List<Candidate> findCandidateBySkillsIsIn(@Param("id")Long id);
